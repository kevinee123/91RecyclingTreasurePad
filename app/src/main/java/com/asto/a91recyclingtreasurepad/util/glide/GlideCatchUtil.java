package com.asto.a91recyclingtreasurepad.util.glide;
...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(
                    AIOApp.Companion.getInstance().getCacheDir() + "/" +
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(AIOApp.Companion.getInstance().getCacheDir() +
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(AIOApp.Companion.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(AIOApp.Companion.getInstance()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(AIOApp.Companion.getInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
